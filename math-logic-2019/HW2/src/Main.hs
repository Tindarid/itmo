        ...
        Right (mapInt, mapExpr, mapMP, mapIntInt, last) -> do
            case (last == theorem) of
                ...
                True -> do
                    ...
                    putStrLn $ firstLine
                    writeLine 1 mapIntInt newMap mapInt used

writeLine n mapIntInt newMap mapInt (a:as) = do
                               putStrLn ((resolvePrint a) ++ (show $ Map.findWithDefault bad a mapInt))
                               writeLine (n + 1) mapIntInt newMap mapInt as
                               where resolvePrint a = "[" ++ show n ++ ". " ++ (r some) ++ "] "
                                     some = Map.findWithDefault (0, 0) a mapIntInt
                                     r (b, c) | b == 0 && c <= 10   = "Ax. sch. " ++ (show c)
                                              | b == 0 && c > 10    = "Hypothesis " ++ (show (c - 10))
                                              | otherwise = "M.P. " ++ (show b1) ++ "," ++ (show c1)
                                              where b1 = Map.findWithDefault 0 b newMap
                                                    c1 = Map.findWithDefault 0 c newMap
