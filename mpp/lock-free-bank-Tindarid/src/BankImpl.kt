...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            /*
            if (account is AcquiredAccount && account.op != op && account.op.completed) {
                continue
            }
             */
            if (account.invokeOperation()) continue
            val acquiredAccount = AcquiredAccount(account.amount, op)
            if (!accounts[index].compareAndSet(account, acquiredAccount)) continue
        ...
        override fun invokeOperation() {
            ...
            while (true) {
                ...
                if (toIndex < fromIndex) {
                    to = acquire(toIndex, this) ?: break
                    from = acquire(fromIndex, this) ?: break
                } else {
                    from = acquire(fromIndex, this) ?: break
                    to = acquire(toIndex, this) ?: break
                }
