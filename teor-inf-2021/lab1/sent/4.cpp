...
struct Predictor {
  ...
  void update(uint bit) {
    ...
    if (ctx_state[ctx] >= states.size()) {
      // std::cout << ctx_state[ctx] << " " << states.size() << std::endl;
...
graph new_graph(graph old) {
  ...
  // int remove_parents = parents[target].size() - 1;
  int remove_parents = rand() % (parents[target].size() - 1) + 1;

  new_g.reserve(old.size() * remove_parents);
  // std::cout << "peek node with count: " << old[target].count[0] << " " << old[target].count[1] << std::endl;
...
constexpr int big_g = 10;
constexpr int ttt = 60 * 60 * 9;

bool in_loop() {
  return time_spented() < ttt;
}

int main(int argc, char** argv) {
  if (argc < 3) return 1;
  start_time = time(nullptr);
  // start_time = 1636486544;
  srand(start_time);

  auto in = read_file(argv[1]);
  auto out = std::ofstream(argv[2]);

  auto initial = recalc(in, fsm0);
  auto initial_total = compress(in, initial);
  auto the_best_states = initial;
  auto the_best_total = initial_total;

  while (in_loop()) {
    auto best_states = initial;
    auto best_total = initial_total;
    // std::cout << "start: " << best_total << std::endl;
    ...
    while (in_loop()) {
      ...
      if (new_g.size() > (1 << 15)) {
        if (big_graph_times++ >= big_g) break;
        continue;
      }
      ...
      // std::cout << "cur : " << best_total << std::endl;
    }

    if (best_total < the_best_total) {
      the_best_total = best_total;
      the_best_states = std::move(best_states);
    }
  }

  flush_states(out, the_best_states);
  // std::cout << "best : " << the_best_total << std::endl;
  return 0;
}
